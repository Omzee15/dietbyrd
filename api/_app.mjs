...
// NOTE: the old POST /api/doctor/patients handler (doctor creates a patient +
// pending-payment booking + direct /pay?ref= link) was removed as dead code:
// zero callers anywhere in the frontend (doctors add patients via
// POST /api/referrals, which sends a self-registration link instead), and it
// referenced dietbyrd_patients.notes / dietbyrd_consultations.referred_by_doctor_id
// / dietbyrd_consultations.notes, none of which exist on this schema, so it
// would have 500'd on every call had anything ever reached it.
